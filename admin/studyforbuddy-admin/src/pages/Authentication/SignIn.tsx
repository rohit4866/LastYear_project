...
const SignIn: React.FC = () => {
  ...
  const [error, setError] = useState(''); // Store error message for Snackbar
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/professors/login`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        const professor_id = data.id; // Store the returned ID in professor_id
        localStorage.setItem('professor_id', professor_id); // Save professor_id in localStorage
        navigate('/dashboard'); // Navigate to the dashboard after successful login
      } else {
        setError(data.message || 'Login failed. Please try again.');
        setSnackbarOpen(true); // Show Snackbar with the error message
      }
    } catch (error) {
          ...
          <div className="hidden w-full xl:block xl:w-1/2">
            <div className="py-17.5 px-26 text-center">
              ...
              <p className="2xl:px-20">
                Signin with StudyForBuddy and get & add internship and placement.
              </p>
              ...
              <form onSubmit={handleLogin}>
                ...
                <div className="mb-6">
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={password}
                      onChange={(e) => setPassword(e.target.value)} // Update password input state
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                    />
                ...
                <div className="mb-5">
                  <input
                    type="submit"
                    value="Sign In"
                    className="w-full cursor-pointer rounded-lg bg-[#365E7D] p-4 text-white transition hover:bg-opacity-90"
                  />
                </div>
